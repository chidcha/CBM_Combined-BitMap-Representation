...
int main(int argc, char** argv)
{
  ...
  for (int l=0; l < num_devices; l++) {
	...
	// copy str substring to substring array
	mb = 0;
...
//Noo                 searchb <<<32*numSMs,1024,0,stream[l] >>>(arr_d_data_b[l], arr_d_substr_b[l],arr_nb[l],strlen(pattern_arr[j]),mb,arr_d_pos[l],total_pattern);
		 
			//searchb_all_shared <<<16*numSMs,128,0,stream[l] >>>(arr_d_data_b[l], arr_d_substr_b[l],arr_nb[l],mb,arr_d_pos[l],total_pattern);
		 
			 searchb_all <<<16*numSMs,128,0,stream[l] >>>(arr_data_b[l],arr_nb[l],mb,arr_d_pos[l],total_pattern);
	...
	for (int l =0; l < num_devices; l++) {
		free(arr_pos[l]);
		cudaFree(arr_data_b[l]);cudaDeviceReset();
	}
